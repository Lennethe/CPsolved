...
struct treedp{
    ...
    void dfs(int idx, int par){
        ...
        for(int& to : g[idx]){
            ...
            depth[p_node] = max(depth[p_node], depth[c_node] + cost[{idx,par}]);
    ...
    void solve(){
        ...
        while(!q.empty()){
            ...
            for(int& to : g[from]){
                ...
                //if(depth[{from,-1}]<=depth[c_node]){
                //    depth[{from,-2}] = depth[{from,-1}];
                //    depth[{from,-1}] = depth[c_node];
                //}
                //else if(depth[{from,-2}]<=depth[c_node]){
                //    depth[{from,-2}] = depth[c_node];
                //}
            }
            //value計算
            for(int& to : g[from])if(!used[to]){
                ...
                //if(depth[{to,from}]==depth[{from,-1}]) depth[{from,to}] = depth[{from,-2}] + cost[{from,to}];
                //else depth[{from,to}] = depth[{from,-1}] + cost[{from,to}];
            }
    ...
    int get_depth(int n){
        ll ret = 0;
        FOR(i,1,n+1){
            ret = max(ret, depth[{i,-1}]+depth[{i,-2}]);
        }
        return ret;
    }
};
